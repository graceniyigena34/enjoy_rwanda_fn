...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-8 h-[70px] transition-colors">
      ...
      {/* Hamburger */}
      <button className="md:hidden text-2xl text-gray-800 dark:text-gray-200 border-none bg-transparent" onClick={() => setMenuOpen(!menuOpen)}>&#9776;</button>

      {/* Mobile actions inside menu */}
      <ul className={`${menuOpen ? "flex" : "hidden"} md:flex flex-col md:flex-row absolute md:static top-[70px] left-0 right-0 bg-white dark:bg-gray-900 md:bg-transparent md:dark:bg-transparent border-b md:border-0 border-gray-200 dark:border-gray-700 shadow-md md:shadow-none list-none m-0 p-0 gap-0 md:gap-8 z-40`}>
        {[["Shop", "/shops"], ["Restaurants", "/restaurants"], ["About", "/about"]].map(([label, path]) => (
          <li key={label}>
            <Link to={path} onClick={() => setMenuOpen(false)} className="block px-8 md:px-0 py-3 md:py-0 text-[0.95rem] font-medium text-gray-500 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white border-b md:border-0 border-gray-100 dark:border-gray-700 transition-colors">
              {label}
            </Link>
          </li>
        ))}
        ...
        <li className="md:hidden flex items-center gap-3 px-8 py-3 border-t border-gray-100 dark:border-gray-700">
          <button onClick={toggleDark} className="flex items-center justify-center w-9 h-9 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors border-none bg-transparent">
          ...
          <Link to="/cart" onClick={() => setMenuOpen(false)} className="relative flex items-center justify-center p-2 rounded-lg text-gray-700 dark:text-gray-300">
          ...
          {user ? (
            <button onClick={() => { logout(); navigate("/"); setMenuOpen(false); }} className="bg-[#1a1a2e] !text-white px-4 py-2 rounded-lg text-sm font-semibold">Sign Out</button>
          ) : (
            <Link to="/login" onClick={() => setMenuOpen(false)} className="bg-[#1a1a2e] !text-white px-4 py-2 rounded-lg text-sm font-semibold">Sign In</Link>
          )}
        </li>
      </ul>
      ...
      <div className="hidden md:flex items-center gap-3">
        ...
        <Link to="/cart" className="relative flex items-center justify-center p-2 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
        ...
        {user ? (
          ...
          <>
            <Link to="/login" className="flex items-center justify-center p-2 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>
              </svg>
            </Link>
