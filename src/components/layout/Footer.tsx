    ...
    <footer className="bg-white border-t border-gray-200 mt-auto">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-10 px-8 py-14">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 leading-relaxed max-w-[240px]">Your gateway to authentic Rwandan experiences. Shop local crafts and discover the best restaurants.</p>
        </div>

        {/* Shop */}
        <div>
          <h4 className="text-sm font-bold text-gray-900 mb-4">Shop</h4>
          <ul className="space-y-2.5">
            {["Imigongo Art","Agaseke Baskets","Coffee & Tea","Jewelry"].map(item => (
              <li key={item}><Link to="/shops" className="text-sm text-gray-500 hover:text-gray-900 transition-colors">{item}</Link></li>
            ))}
          </ul>
        </div>
        ...
        {/* Restaurants */}
        <div>
          <h4 className="text-sm font-bold text-gray-900 mb-4">Restaurants</h4>
          <ul className="space-y-2.5">
            {["Kigali","Musanze","Rubavu","Huye"].map(city => (
              <li key={city}><Link to="/restaurants" className="text-sm text-gray-500 hover:text-gray-900 transition-colors">{city}</Link></li>
            ))}
          </ul>
        </div>
      ...
      <div className="border-t border-gray-200 max-w-6xl mx-auto px-8 py-4 flex flex-col sm:flex-row justify-between items-center gap-2 text-xs text-gray-400">
