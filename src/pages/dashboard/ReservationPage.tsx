...
export default function ReservationPage() {
  ...
  const handleAddRow = async () => {
    ...
    try {
      const updated = [
        ...configs.map(({ table_of_people, price }) => ({
          table_of_people,
          price,
        })),
  ...
  return (
    <section className="page-shell space-y-8 pb-16">
        ...
        <div className="flex items-start justify-between gap-6">
          <div className="flex-1">
            ...
            <p className="mt-3 text-lg text-slate-600 dark:text-slate-300 max-w-2xl leading-relaxed">
              Define your table configurations and pricing. Customers will
              instantly see these options when making a dining reservation.
            </p>
          </div>
          <button
            type="button"
            onClick={() => void loadConfigs()}
            className="group inline-flex items-center gap-2 rounded-xl border-2 border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 shadow-lg shadow-slate-200/20 transition-all hover:border-orange-400 hover:shadow-orange-300/30 dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:hover:border-orange-500 dark:hover:shadow-orange-500/20"
          >
            <span className="transition group-hover:rotate-180 inline-block">
              ↻
            </span>{" "}
            Refresh
          </button>
      ...
      {msg && (
        <div
          className={`animate-slideDown rounded-2xl px-5 py-4 text-sm font-bold border-2 backdrop-blur-xl flex items-center gap-3 ${msg.type === "success" ? "bg-emerald-50/80 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-300 dark:border-emerald-800" : "bg-red-50/80 text-red-700 border-red-200 dark:bg-red-900/20 dark:text-red-300 dark:border-red-800"}`}
        >
        ...
        <div className="rounded-3xl border-2 border-slate-200/60 bg-gradient-to-br from-white via-slate-50/40 to-white/80 p-8 shadow-2xl shadow-slate-200/40 backdrop-blur-2xl transition-all hover:shadow-2xl hover:shadow-orange-300/20 dark:border-white/10 dark:from-slate-900/60 dark:via-slate-800/40 dark:to-slate-900/40 dark:shadow-slate-900/60">
          <div className="flex items-center justify-between mb-8">
            <div className="flex-1">
              ...
              <p className="mt-2 text-base text-slate-500 dark:text-slate-400">
                Create and manage your restaurant's table offerings
              </p>
            </div>
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-orange-400 via-orange-500 to-orange-600 shadow-lg shadow-orange-500/30">
              <svg
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="mt-6 text-base font-bold text-slate-600 dark:text-slate-400">
                Fetching your tables...
              </p>
            ...
            <div className="overflow-hidden rounded-2xl border-2 border-slate-200/50 dark:border-white/5 shadow-inner shadow-slate-200/20">
              <table className="w-full text-left">
                <thead className="sticky top-0 bg-gradient-to-r from-slate-100 via-slate-50 to-white dark:from-white/8 dark:via-white/5 dark:to-transparent border-b-2 border-slate-200/50 dark:border-white/5 backdrop-blur-sm">
                  <tr>
                    <th className="px-6 py-5 text-xs font-black uppercase tracking-widest text-slate-700 dark:text-slate-300">
                      ID
                    </th>
                    <th className="px-6 py-5 text-xs font-black uppercase tracking-widest text-slate-700 dark:text-slate-300">
                      Configuration
                ...
                <tbody className="divide-y divide-slate-200/40 dark:divide-white/5">
                  {configs.length === 0 ? (
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
                              Start by adding your first table configuration
                              below
                            </p>
                          </div>
                        </div>
                    ...
                    configs.map((config, index) => (
                      <tr
                        key={config.id}
                        className="table-row-hover group/row bg-gradient-to-r from-white/50 via-white/30 to-transparent transition-all hover:from-orange-50/50 hover:via-orange-50/30 dark:from-white/3 dark:via-white/2 dark:to-transparent dark:hover:from-orange-500/10 dark:hover:via-orange-500/5"
                      >
                        ...
                        <td className="px-6 py-5">
                          {editingId === config.id ? (
                            <input
                              type="text"
                              value={editForm.table_of_people}
                              onChange={(e) =>
                                setEditForm((f) => ({
                                  ...f,
                                  table_of_people: e.target.value,
                                }))
                              }
                              className="rounded-xl border-2 border-orange-400 bg-white/80 px-4 py-2.5 text-sm font-bold text-slate-900 outline-none transition focus:ring-2 focus:ring-orange-400/30 dark:bg-white/10 dark:border-orange-500 dark:text-white"
                              placeholder="e.g. 2-4"
                              autoFocus
                            />
                          ) : (
                            ...
                            {editingId === config.id ? (
                              <>
                                <button
                                  type="button"
                                  disabled={saving}
                                  onClick={() => void handleEditSave(config.id)}
                                  className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-emerald-500 to-emerald-600 px-4 py-2 text-xs font-bold text-white shadow-lg shadow-emerald-500/30 transition hover:shadow-emerald-500/50 disabled:opacity-50"
                                >
                                  ✓ Save
                                </button>
                                <button
                                  ...
                                  className="rounded-lg border-2 border-slate-300 bg-white/80 px-4 py-2 text-xs font-bold text-slate-700 transition hover:bg-slate-50 dark:border-white/10 dark:bg-white/5 dark:text-slate-300 dark:hover:bg-white/10"
                                >
                                  ✕ Cancel
                                </button>
                              </>
                              ...
                              <>
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    setEditForm({
                                      table_of_people: config.table_of_people,
                                      price: String(config.price),
                                    });
                                  }}
                                  className="rounded-lg border-2 border-[#1a1a2e] bg-white px-4 py-2 text-xs font-bold text-[#1a1a2e] shadow-sm transition hover:bg-[#1a1a2e] hover:text-white dark:bg-white/5 dark:border-orange-500 dark:text-orange-400 dark:hover:bg-orange-500/20 dark:hover:border-orange-600"
                                >
                                  ✎ Edit
                                </button>
                                <button
                                  ...
                                  className="rounded-lg border-2 border-red-300 bg-white px-4 py-2 text-xs font-bold text-red-600 shadow-sm transition hover:bg-red-50 disabled:opacity-50 dark:border-red-900/50 dark:bg-red-900/10 dark:text-red-400 dark:hover:bg-red-900/30"
                                >
          ...
          <div className="mt-8 space-y-6 border-t-2 border-slate-200/50 pt-8 dark:border-white/5">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
                Create a new table configuration for your restaurant
              </p>
            </div>
        ...
        <div className="rounded-3xl border-2 border-slate-200/60 bg-gradient-to-br from-white via-slate-50/40 to-white/80 p-8 shadow-2xl shadow-slate-200/40 backdrop-blur-2xl transition-all hover:shadow-2xl hover:shadow-orange-300/20 dark:border-white/10 dark:from-slate-900/60 dark:via-slate-800/40 dark:to-slate-900/40 dark:shadow-slate-900/60">
          <div className="flex items-center justify-between mb-8">
            <div className="flex-1">
              ...
              <p className="mt-2 text-base text-slate-500 dark:text-slate-400">
                How customers see your tables when booking
              </p>
            </div>
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-orange-400 via-orange-500 to-orange-600 shadow-lg shadow-orange-500/30 text-2xl">
              👁️
            </div>
          </div>
          {configs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-2">
                Add your first table above to see a live preview
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="relative space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-3xl font-black text-[#1a1a2e] dark:text-orange-400 mt-1">
                        {config.table_of_people}
                      </p>
                    </div>
