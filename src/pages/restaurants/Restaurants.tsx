...
export default function Restaurants() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-8 py-10">
      <div className="mb-8">
        <h1 className="text-3xl font-black text-gray-900 dark:text-white mb-1">Restaurants in Rwanda</h1>
        <p className="text-gray-500 dark:text-gray-400">Discover and book the best dining experiences</p>
      </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {cuisines.map(c => (
            <button key={c} onClick={() => setCuisine(c)}
              className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${cuisine === c ? "bg-[#1a1a2e] text-white border-[#1a1a2e]" : "bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:border-gray-800 dark:hover:border-gray-400"}`}>
              {c}
            </button>
          ))}
        </div>
          ...
          <Link to={`/restaurants/${r.id}`} key={r.id} className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-200 no-underline">
            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-1">
                <h3 className="font-bold text-gray-900 dark:text-white text-base">{r.name}</h3>
                <span className="text-xs bg-blue-50 dark:bg-blue-900 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full font-medium">{r.cuisine}</span>
              </div>
